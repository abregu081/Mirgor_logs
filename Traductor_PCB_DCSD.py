...
def es_fecha_valida(start_time, formato='%H:%M:%S'):
    try:
        datetime.strptime(start_time, formato)
        return True
    except ValueError:
        return False
...
for carpeta_fecha in os.listdir(input_dir):
    ...
    if os.path.isdir(ruta_carpeta_fecha) and carpeta_fecha.isdigit() and len(carpeta_fecha) == 6:
        ...
        for estado in ["FAIL", "PASS"]:
            ...
            if os.path.exists(carpeta_estado):
                for subcarpeta in os.listdir(carpeta_estado):
                    ruta_subcarpeta = os.path.join(carpeta_estado, subcarpeta)
                            ...
                            if archivo.endswith(".csv"):
                                ...
                                try:
                                    with open(ruta_archivo, mode="r", encoding="ISO-8859-1") as f:
                                        ...
                                        step_found = False  # Variable para verificar si encontramos un step
                                        for fila in csv_reader:
                                            # Detectamos el inicio de un nuevo bloque de datos
                                            if fila and fila[0] == "TEST NAME":
                                                header_skipped = True
                                            
                                            # Asegurarse de procesar solo si hay datos importantes (por ejemplo, barcode o start_time)
                                            if header_skipped and (fila[8] or fila[9]):  # Revisa si hay barcode o start_time
                                                try:
                                                    test_name, start_time, total_time, barcode = fila[0], fila[9], fila[11], fila[8]
                                                except Exception as e:
                                                    print(f"Error al procesar la fila {fila}: {e}")
                                                    continue
                                                if not es_fecha_valida(start_time):
                                                    continue

                                                # Si es un archivo en "PASS"
                                                ...
                                                # Si es un archivo en "FAIL", verificar si encontramos NG
                                                    ...
                                                    step = "N/A"  # Asignamos N/A si no se encuentra NG
                                                    result = "FAIL"
        ...
        print(f"Advertencia: {ruta_carpeta_fecha} no es una carpeta de fecha válida.")
